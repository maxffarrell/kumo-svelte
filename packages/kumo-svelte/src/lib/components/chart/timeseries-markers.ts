...
export function clusterTimeseriesMarkers(
  ...
  for (const marker of sorted) {
    ...
    if (previous && marker.timestamp - previous[previous.length - 1].timestamp <= interval) {
...
export function getApproximateMarkerClusterInterval(timestamps: number[], tickCount: number): number {
