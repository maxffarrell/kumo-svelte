...
export function buildTimeseriesThresholdAnnotations(thresholds: TimeseriesThreshold[] | undefined) {
  ...
  return {
    markLine: {
      ...
      blur: { lineStyle: { opacity: 1 }, label: { opacity: 1 } },
      label: {
        show: true,
      ...
      data: thresholds.map((threshold) => ({
        ...
        lineStyle: { color: threshold.color, width: 1, type: 'dashed' as const },
        blur: { lineStyle: { opacity: 1 }, label: { opacity: 1 } },
        label: { show: Boolean(threshold.label), color: threshold.color }
      }))
    }
  };
}
