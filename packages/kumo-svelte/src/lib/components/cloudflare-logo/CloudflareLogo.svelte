<script lang="ts" module>
  ...
  export const KUMO_CLOUDFLARE_LOGO_VARIANTS = {
    variant: {
      glyph: { description: 'Cloud glyph only (logomark)' },
      full: { description: 'Full logo with cloud glyph and wordmark stacked' }
    },
    color: {
      color: { description: 'Brand colors (orange/yellow gradient cloud, dark gray text)' },
      black: { description: 'Solid black logo' },
      white: { description: 'Solid white logo (for dark backgrounds)' }
    }
  } as const;
...
<script lang="ts">
  ...
  const isGlyph = $derived(variant === 'glyph');
  const fillOrange = $derived(color === 'color' ? CLOUDFLARE_ORANGE : 'currentColor');
  const fillYellow = $derived(color === 'color' ? CLOUDFLARE_YELLOW : 'currentColor');
  const logoClass = $derived(
    cn(
      color === 'color' && !isGlyph && 'text-kumo-default',
...
{:else}
  ...
  >
    <path d={CLOUDFLARE_FULL_LOGO_ORANGE_PATH} fill={fillOrange} />
    <path d={CLOUDFLARE_FULL_LOGO_YELLOW_PATH} fill={fillYellow} />
    {#each CLOUDFLARE_WORDMARK_PATHS as path (path)}
