...
describe("ToastPreview", () => {
  it("renders with Kumo data attributes", () => {
    ...
    expect(
      document.querySelector('[data-kumo-component="Toast"]'),
    ).toBeTruthy();
  });
  ...
  describe("variant fidelity", () => {
    ...
    it("applies success variant ring and title color selectors", () => {
      ...
      expect(root?.className).toContain(
        "[&_[data-toast-title]]:text-kumo-success",
      );
    });
    ...
    it("uses the current subtle token for description text", () => {
    ...
    it("applies tint background for success variant", () => {
      ...
      const background = document.querySelector(
        '[data-kumo-component="Toast"] .absolute.inset-0',
      );
