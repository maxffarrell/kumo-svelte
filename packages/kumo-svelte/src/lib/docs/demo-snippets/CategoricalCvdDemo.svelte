<script lang="ts">
  ...
  import { categoricalColorIndices, getIsDarkMode, simulateCvdHex } from './chart-color-demo-data';

  let isDarkMode = $state(false);
  const simulatedColors = $derived(
    categoricalColorIndices.map((index) => simulateCvdHex(ChartPalette.categorical(index, isDarkMode)))
  );
