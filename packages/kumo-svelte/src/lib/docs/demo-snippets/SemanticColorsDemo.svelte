<script lang="ts">
  ...
  const colorFor = (name: (typeof semanticColorNames)[number]) => semantic(name, isDarkMode);
          ...
          {#each semanticColorNames as name}
            <Table.Cell class="w-1/6">
              <div class="flex items-center gap-2">
                <div style:background-color={colorFor(name)} class="size-5 rounded"></div>
                <span class="font-mono text-xs">{colorFor(name)}</span>
              </div>
            </Table.Cell>
          {/each}
        </Table.Row>
